...
def reset_level():
    ...
    return world
def draw_text(text, colour, size,x ,y):
        ...
        for row in data:
            ...
            for tile in row:
                if tile == 1 or tile == 2 or tile == 3 or tile == 4:
                    images = {1:dirt_img1, 2:grass_img2, 3:swamp_img3, 4:sand_img4}
                    ...
                    exit_ = Exit(col_count * tile_size,
                                 row_count * tile_size + (tile_size // 2))
                    exit_group.add(exit_)
                elif tile == 6:
                    diamonds = Diamonds(col_count * tile_size + (tile_size//2),
                                        row_count * tile_size + (tile_size//2))
                    diamond_group.add(diamonds)
                col_count += 1
            row_count += 1
    def draw(self):
...
class Diamonds(pygame.sprite.Sprite):
    def __init__(self, x, y):
        super().__init__()
        img = pygame.image.load("Images/6diamond.png")
...
while run:
    ...
    if main_menu:
        ...
        draw_text(str(score), (255,255,255), 20, 10, 35)
        player.update()
        lava_group.update()
